...
export abstract class SPFxAppDevBaseWebPart<
  TProperties
> extends BaseClientSideWebPart<TProperties> {
  ...
  protected onInit(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      super.onInit().then(() => {
        const ctx: BaseComponentContext = this
          .context as any as BaseComponentContext;
        this.helper = {
          url: new SPUri(ctx),
          functions: new GeneralHelper(),
        };

        this.spfxContext = ctx;
...
export abstract class SPFxAppDevClientSideWebPart<
  WPProps = ISPFxAppDevClientSideWebPartProps
> extends SPFxAppDevBaseWebPart<WPProps> {
  ...
  protected onInit(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      super.onInit().then(() => {
        //TODO: check why compiler throws error, when using without any...
        this.SettingsReader = this.context.serviceScope.consume(
          SettingsReaderService.serviceKey as any
        );
        this.Bootloader = this.context.serviceScope.consume(
          BootLoaderService.serviceKey as any
        );

        this.Bootloader.onLoad(this.getRequiredDataForBoot())
          .then(() => {
            return resolve();
          })
          .catch((error) => {
  ...
  protected getRequiredSettings(): SettingsLoader<ISettings>[] {
    const generalSettings: SettingsLoader<IGeneralSettings> = {
      ...
      onLoaded: (settings: IGeneralSettings) => {
        ...
        Logger.DefaultSettings.LogLevel =
          this.GeneralSettings.ConsoleLoggingLevel;
      },
    };
  ...
  private loadRequiredSettings(): Promise<void> {
    ...
    allSettings.forEach((settings: SettingsLoader<ISettings>) => {
      allPromises.push(
        this.SettingsReader.getSettings(
          settings.key,
    ...
    return new Promise<void>((resolve, reject) => {
      Promise.all(allPromises).then((allSettingsFromReader: ISettings[]) => {
        allSettings.forEach(
          (settings: SettingsLoader<ISettings>, index: number) => {
